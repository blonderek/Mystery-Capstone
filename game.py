...
class Player(Character):
    def __init__(self):
        ...
        self.health_max = 10
    def quit(self):
        print "%s waves as he closes the door." % self.name
        print "with a familiar sound, the TARDIS pulsates and disappears."
        self.health = 0
    def help(self):
...
print "%s opens the TARDIS door and peers out, wondering what's in store." % p.name

while(p.health > 0):
    line = raw_input("What would you like to do? ")
    args = line.split()
    if len(args) > 0:
        commandFound = False
